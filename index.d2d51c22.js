...
const toggleTabs = (target)=>{
    ...
    tabsTxt.forEach((e)=>{
        e.classList.remove('active');
        if (e.dataset.id === target.dataset.id) e.classList.add('active');
    });
};
...
const windowClick = ($evt)=>{
    ...
    if (target.closest('.tab')) toggleTabs(target);
    else if (target.closest('.qa-container')) target.closest('.qa-container').classList.toggle('active');
    else if (target.closest('.anchor-link')) scrollTo(target);
    else if (target.closest('.footer-button')) window.scrollTo({
        top: 0,
        behavior: 'smooth'
    });
};
...
//# sourceMappingURL=index.d2d51c22.js.map
